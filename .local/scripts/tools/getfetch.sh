...
fastfetch -s gpu --gpu-detection-method vulkan --pipe -l none | sed 's/GPU/FineWine™ Encoders/g; s/RADV //g; s/\[Discrete\]//g' > "$CACHEDIR/fastfetch/gpuinfo"
fastfetch -s vulkan --pipe -l none | sed 's/Vulkan //g' > "$CACHEDIR/fastfetch/vulkaninfo"

sleep 3 # Wait a bit for internet
wget -q --spider "wttr.in/Maracaibo,VE?0TF" || { sleep 10 && wget --spider "wttr.in/Maracaibo,?0TF" || notify-send -u critical "Cannot connect to weather server"; } 
wget -q "wttr.in/Maracaibo?0TF" -O - | sed -E 's/^.{15}//g; /^$/d;' | awk 'NR<5 && NR>1 {$1=$1; printf "%s ", $0}' > "$CACHEDIR/weatherinfo"
