...
if ! command -v cpupower 1>/dev/null || ! command -v dunstify 1>/dev/null; then
	awk 'BEGIN {print "You need cpupower and dunstify for this script to work"}'
	notify-send "Missing dependency!"
	exit 1
fi

_governor () {
	GOVERNOR=$(awk '{print $0}' /sys/devices/system/cpu/cpu0/cpufreq/scaling_governor 2>/dev/null)
}

_notify () {
	[ -z $GOVERNOR ] && exit 1
	awk -v v="CPU scheduler set: $GOVERNOR" 'BEGIN { print v }'
